            ...
            if (typeof featureValue === 'boolean') {
                if (!featureValue) {
                    ...
                    Promise.resolve().then(() => __importStar(require('../services/audit.service'))).then(({ auditService }) => {
                        auditService.logPlanLimitReached(req.user.id, req.user.email, `feature:${feature}`).catch(err => console.error('[AUDIT]', err));
                    });
                    res.status(403).json({
        ...
        try {
            ...
            if (typeof limit === 'number' && limit !== -1) {
                ...
                if (currentCount >= limit) {
                    // Audit log - plan limit reached
                    Promise.resolve().then(() => __importStar(require('../services/audit.service'))).then(({ auditService }) => {
                        auditService.logPlanLimitReached(req.user.id, req.user.email, `${limitKey}:${currentCount}/${limit}`).catch(err => console.error('[AUDIT]', err));
                    });
                    res.status(403).json({
