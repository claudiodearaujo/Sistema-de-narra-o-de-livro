...
async function setup() {
    ...
    testUser2 = await prisma_1.default.user.create({
        data: {
...
async function runTests() {
    ...
    runner.addTest('getBalance returns 0 for new user', async () => {
        const balance = await livra_service_1.livraService.getBalance(testUser.id);
    ...
    runner.addTest('addLivras increases balance', async () => {
        const result = await livra_service_1.livraService.addLivras(testUser.id, {
            type: client_1.LivraTransactionType.EARNED_LIKE,
            amount: 10,
            metadata: { test: true },
        });
    ...
    runner.addTest('addLivras accumulates balance', async () => {
        await livra_service_1.livraService.addLivras(testUser.id, {
            type: client_1.LivraTransactionType.EARNED_FOLLOW,
            amount: 5,
        });
        const balance = await livra_service_1.livraService.getBalance(testUser.id);
    ...
    runner.addTest('hasSufficientBalance returns correct result', async () => {
        const hasEnough = await livra_service_1.livraService.hasSufficientBalance(testUser.id, 10);
        runner.assertTrue(hasEnough, 'Should have sufficient balance for 10');
        const notEnough = await livra_service_1.livraService.hasSufficientBalance(testUser.id, 100);
    ...
    runner.addTest('spendLivras decreases balance', async () => {
        const result = await livra_service_1.livraService.spendLivras(testUser.id, {
            type: client_1.LivraTransactionType.SPENT_TTS,
            amount: 5,
    ...
    runner.addTest('spendLivras fails with insufficient balance', async () => {
        ...
        try {
            await livra_service_1.livraService.spendLivras(testUser.id, {
                type: client_1.LivraTransactionType.SPENT_TTS,
                amount: 1000,
            });
        }
            ...
            errorThrown = true;
            runner.assertTrue(error.message.includes('Insufficient') || error.message.includes('saldo') || error.message.includes('insuficiente'), 'Should throw insufficient balance error');
        }
    ...
    runner.addTest('getTransactionHistory returns transactions', async () => {
        const history = await livra_service_1.livraService.getTransactionHistory(testUser.id, 1, 10);
    ...
    runner.addTest('getTransactionHistory filters by type', async () => {
        const history = await livra_service_1.livraService.getTransactionHistory(testUser.id, 1, 10, { type: client_1.LivraTransactionType.EARNED_LIKE });
        runner.assertNotNull(history, 'History should not be null');
        ...
        for (const tx of history.transactions) {
            runner.assertEqual(tx.type, client_1.LivraTransactionType.EARNED_LIKE, 'Type should match filter');
        }
    });
    // Test 9: Get config value
    runner.addTest('getConfigValue returns default values', async () => {
        const likeReward = await livra_service_1.livraService.getConfigValue('LIKE_RECEIVED');
        runner.assertGreaterThan(likeReward, 0, 'LIKE_RECEIVED should be positive');
        const ttsCost = await livra_service_1.livraService.getConfigValue('TTS_COST');
    ...
    runner.addTest('awardForLikeReceived adds correct amount', async () => {
        const balanceBefore = await livra_service_1.livraService.getBalance(testUser.id);
        await livra_service_1.livraService.awardForLikeReceived(testUser.id, 'test-post-id', testUser2.id);
        const balanceAfter = await livra_service_1.livraService.getBalance(testUser.id);
