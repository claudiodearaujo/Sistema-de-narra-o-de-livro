...
class MessageService {
     ...
     */
    async getConversations(userId, page = 1, limit = CONVERSATIONS_PER_PAGE) {
     ...
     */
    async getMessages(userId, otherUserId, page = 1, limit = MESSAGES_PER_PAGE) {
        const skip = (page - 1) * limit;
    ...
    async sendMessage(senderId, receiverId, dto) {
        ...
        setImmediate(async () => {
            try {
                await achievement_service_1.achievementService.checkAndUnlock(senderId, 'messages_sent');
            }
            catch (err) {
    ...
    async markAsRead(userId, otherUserId) {
        const result = await prisma_1.default.message.updateMany({
            where: {
                senderId: otherUserId,
                receiverId: userId,
                isRead: false
     ...
     */
    async createMessageNotification(receiverId, senderId, content) {
        try {
