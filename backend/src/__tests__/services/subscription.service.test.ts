...
async function getUserPlan(userId: string): Promise<SubscriptionPlan | 'FREE'> {
...
function getPlanFeatures(plan: SubscriptionPlan | 'FREE') {
...
async function runTests() {
  ...
  runner.addTest('premium user has PREMIUM subscription', async () => {
    ...
    runner.assertEqual(subscription!.plan, SubscriptionPlan.PREMIUM, 'Plan should be PREMIUM');
    runner.assertEqual(subscription!.status, SubscriptionStatus.ACTIVE, 'Status should be ACTIVE');
  });
  ...
  runner.addTest('pro user has PRO subscription', async () => {
    ...
    runner.assertNotNull(subscription, 'Subscription should exist');
    runner.assertEqual(subscription!.plan, SubscriptionPlan.PRO, 'Plan should be PRO');
  });
