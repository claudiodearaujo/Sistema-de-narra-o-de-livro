...
async function runTests() {
  ...
  runner.addTest('create() should fail for short title', async () => {
    try {
      ...
      runner.assertTrue(
        ...
        error.message.toLowerCase().includes('3'),
        'Should mention title validation'
      );
    }
  });
  ...
  runner.addTest('create() should fail for missing author', async () => {
    try {
      ...
      runner.assertTrue(
        error.message.toLowerCase().includes('author'),
        'Should mention author is required'
      );
    }
  });
  ...
  runner.addTest('getById() should return the book with chapters', async () => {
    const book = await booksService.getById(createdBookId);
  ...
  runner.addTest('getById() should throw for non-existent book', async () => {
    try {
      await booksService.getById('non-existent-id');
      throw new Error('Should have thrown an error');
    } catch (error: any) {
      runner.assertTrue(
        error.message.toLowerCase().includes('not found'),
        'Should mention book not found'
      );
    }
  });
  ...
  runner.addTest('update() should update book data when user is owner', async () => {
    const updated = await booksService.update(createdBookId, {
  ...
  runner.addTest('update() should fail when user is not owner', async () => {
    try {
      await booksService.update(createdBookId, {
      ...
      runner.assertTrue(
        ...
        error.message.toLowerCase().includes('own'),
        'Should mention unauthorized access'
      );
    }
  });
  ...
  runner.addTest('getAll() should return paginated books for specific user', async () => {
    const result = await booksService.getAll(1, 10, undefined, undefined, testUserId1);
    ...
    runner.assertTrue(
      result.data.every((b: any) => b.userId === testUserId1),
  ...
  runner.addTest('getAll() should filter by title and userId', async () => {
    const result = await booksService.getAll(1, 10, 'Test Book Updated', undefined, testUserId1);

    runner.assertGreaterThan(result.data.length, 0, 'Should find at least one book');
    runner.assertTrue(
      result.data.some((b: any) => b.title.includes('Test Book')),
  ...
  runner.addTest('getAll() should return empty for user with no books', async () => {
    const result = await booksService.getAll(1, 10, undefined, undefined, testUserId2);
  ...
  runner.addTest('getStats() should return book statistics', async () => {
    const stats = await booksService.getStats(createdBookId);
  ...
  runner.addTest('delete() should fail when user is not owner', async () => {
    try {
      await booksService.delete(createdBookId, testUserId2);
      throw new Error('Should have thrown an error');
    } catch (error: any) {
      runner.assertTrue(
        error.message.toLowerCase().includes('unauthorized') ||
  ...
  runner.addTest('delete() should remove the book when user is owner', async () => {
    const result = await booksService.delete(createdBookId, testUserId1);
  ...
  runner.addTest('delete() should throw for non-existent book', async () => {
    try {
      await booksService.delete('non-existent-id', testUserId1);
      throw new Error('Should have thrown an error');
    } catch (error: any) {
      runner.assertTrue(
        error.message.toLowerCase().includes('not found'),
